...
Palette.prototype.getColor = function(_x, _y) {
  return this.marbleVbo[Math.floor(_x / this.paletteScaleFactor)][
...
Palette.prototype.createMarble = function() {
  for (var x = 0; x < this.palWidth; x++) {
    var hueVar =
      this.c +
      this.map(
        customNoise.noise(this.hueOffset),
        0,
        1,
        -this.hueRange,
        this.hueRange
    ...
    for (var y = 0; y < this.palHeight; y++) {
      var xyValue =
        (x + this.randomXoffset) * this.xPeriod / this.palWidth +
        (y + this.randomYoffset) * this.yPeriod / this.palHeight +
        this.turbPower *
          customNoise.noise(x / this.turbSize, y / this.turbSize);
      var sineValue = Math.abs(Math.sin(xyValue * 3.14159));
      var tempColor = [
        hueVar,
        1 - sineValue,
        this.map(sineValue, 0, 1, this.minMarbleBrightness, 1)
      ];
